...
(defrule my-rule11
	(customer (name ?n)) =>
	(printout t "Customer name found:" ?n  crlf )
)
...
(defrule my-rule12
	(customer (customer-id 101) (name ?name) (address ?ad)) =>
	(printout t "customer-id 101 belongs to:: " ?name " with address:: " ?ad crlf)
)
...
;;(defrule my-rule14
;;	(not (product (category smartphone) (price ?p) (name ?n)))
;;	(test(< ?p 50))=>
